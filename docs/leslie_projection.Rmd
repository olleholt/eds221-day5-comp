...
# The line below... create a sequence from 0 to 8 years in increments of 1 year
time_yr <- seq(from = 0, to = 8, by = 1)

# The line below... create a matrix for the projection years. With length of the
# initial population vector for columns and the time vector for rows.
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# The line below... populates the empty matrix (proj_year) with the initial
# populations from fish_year0
proj_year[1, ] <- fish_year0

# The for loop below... creating a for loop with length of 2 to the length of 
# the time_year vector. Filling the proj_year matrix with the dot product of
# the initial leslie matrix with the proj_year data from the previous year
# (i-1)
...
#reassigning the column names in the matrix

proj_df <- data.frame(time_yr, proj_year) %>% #populate each row with the year
  pivot_longer(cols = -time_yr, names_to = "lifestage", values_to = "stage_population")
#pivot_longer ceates a df with teo columns, all lifestages are in one column and
# all populations are in the other column
